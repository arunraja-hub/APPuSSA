...
# tf.data.Dataset.from_tensor_slices((train_input_tensor,train_target_tensor)).batch(BATCH_SIZE, drop_remainder=False)
...
class Decoder(Layer):
    ...
    def __init__(self, **kwargs):
        ...
        self.embedding = Embedding(vocab_tar_size, 512, mask_zero=True)
    ...
    @tf.function
    def call(self, inputs,enc_states, training = None):
...
encoder_states = [h_forward, c_forward]

dec_input = Input(shape=(None,))
decoder = Decoder()
decoder_outputs = decoder(dec_input,encoder_states)
