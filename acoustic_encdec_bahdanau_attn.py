...
def get_time_boundaries_from_ctm_file(audio,path):
    ...
    for uttid in time_boundaries_uttids:
        ...
        time_boundaries[uttid] = [[i,time_boundaries[uttid].numpy()[k]] for k in range(time_boundaries[uttid].numpy().shape[0])]
...
class HierarchicalEncoder(tf.keras.Model):
    def __init__(self,enc_units, batch_size):
        ...
        # self.masker = tf.keras.layers.Masking()
...
def bucketer(input_ds,target_ds,BATCH_SIZE):
    ...
    def generator():
        for i in input_ds:
            yield (np.array(input_ds[i][0], dtype = np.float32),np.array(input_ds[i][1]),np.array(target_ds[i])[:-2]), np.array(target_ds[i])[1:-1]
    ...
    print('dataset')
    dataset = tf.data.Dataset.from_generator(generator, ((tf.float32, tf.int64,tf.int64),tf.int64), (([None,43],[None], [None]),[None]))
    ...
    #     print(ds[i])
        # print(i)
        print(i[0][0].shape,i[0][1].shape,i[0][2].shape,i[1].shape)
        c+=1
...
class Decoder(Layer):
    ...
    def decoder_loop(self,decoder_inputs,states):
          ...
          print('decoder loop inputs-----',tf.shape(decoder_inputs),tf.shape(states),tf.shape(states[2]))
...
if __name__ == '__main__':
    ...
    # print('train and val lengths are:')
    ...
    decoder_outputs = decoder(dec_input,enc_hidden,enc_output)
    
    model = Model([inp,bounds, dec_input], decoder_outputs)
